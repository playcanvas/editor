...
test('an entity created by another account appears as a collaborator row', async ({ editorPage, collaborator, project }) => {
...
test('a rename in one account updates the collaborator row text', async ({ editorPage, collaborator, project }) => {
    test.skip(!collaborator, SKIP);
    const guest = await join(editorPage, collaborator, project, 'write');
...
test('a delete in one account removes the collaborator row', async ({ editorPage, collaborator, project }) => {
    test.skip(!collaborator, SKIP);
    const guest = await join(editorPage, collaborator, project, 'write');
    const host = new HierarchyPanel(editorPage);
    const hierarchy = new HierarchyPanel(guest.page);
...
test('a remote selection marks the entity row in the collaborator hierarchy', async ({ editorPage, collaborator, project }) => {
    test.skip(!collaborator, SKIP);
    const guest = await join(editorPage, collaborator, project, 'write');
    const host = new HierarchyPanel(editorPage);
    const hierarchy = new HierarchyPanel(guest.page);
...
test('a collaborator joining and leaving updates the who-is-online strip', async ({ editorPage, collaborator, project }) => {
    test.skip(!collaborator, SKIP);
...
test('losing the connection raises the overlay and reconnecting clears it', async ({ editorPage, collaborator, project }) => {
    test.skip(!collaborator, SKIP);
    ...
    await guest.page.waitForFunction(() => {
        return window.editor.api.globals.realtime.connection.authenticated;
    }, null, { timeout: RECONNECT_TIMEOUT });
});
