...
import { createEsmScript, waitForParser } from '../../lib/common';
...
const setText = async (page: Page, id: number, text: string) => {
    const hashed = await arm(page, (assetId: number) => {
        const asset = window.editor.api.globals.assets.get(assetId) as any;
        const hash = asset.get('file.hash');
        // a re-upload replaces the whole file object, so take every write and re-read the hash
        return { done: new Promise<void>((resolve) => {
            const evt = asset.on('*:set', () => {
                if (asset.get('file.hash') === hash) {
                    return;
                }
                evt.unbind();
                resolve();
...
test.describe('scripting', () => {
    ...
    test('create ESM script', async ({ editorPage }) => {
    ...
    test('create classic script', async ({ editorPage }) => {
        test.setTimeout(JOB_TEST_TIMEOUT);
        const assets = new AssetsPanel(editorPage);
    ...
    test('report invalid attribute', async ({ editorPage, errors }) => {
        test.setTimeout(JOB_TEST_TIMEOUT);
    ...
    test('reorder scripts', async ({ editorPage }) => {
        test.setTimeout(JOB_TEST_TIMEOUT);
        const hierarchy = new HierarchyPanel(editorPage);
        const inspector = new Inspector(editorPage);
    ...
    test('register parse method', async ({ editorPage }) => {
        test.setTimeout(JOB_TEST_TIMEOUT);
        const assets = new AssetsPanel(editorPage);
