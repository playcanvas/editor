editor.once('load', () => {
    ...
    editor.method('scenes:list', (callback?: (result: unknown) => void) => {
        editor.api.globals.rest.projects.projectScenes().on('load', (_status: number, data: { result: unknown }) => {
            if (callback) {
    ...
    editor.method('scenes:new', (name: string, callback?: (data: unknown) => void) => {
        ...
        editor.api.globals.rest.scenes.sceneCreate(data).on('load', (_status: number, data: unknown) => {
            if (callback) {
                callback(data);
            }
        });
    ...
    editor.method('scenes:delete', (sceneId: string, callback?: () => void) => {
        editor.api.globals.rest.scenes.sceneDelete(sceneId).on('load', (_status: number, _data: unknown) => {
            if (callback) {
                callback();
            }
        });
