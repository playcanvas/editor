...
class AnimstategraphLayers extends Panel {
    constructor(parent: Panel, args: Record<string, unknown>) {
        ...
        this._layerBase = new Container();
        this.append(this._layerBase);

        this._layerSort = new Container();
        this._layerSort.on('child:dragend', this._onDragEnd.bind(this));
        this.append(this._layerSort);
    ...
    _onDragEnd(inspector: unknown, sortedNewId: number, sortedOldId: number) {
        ...
        // offset id by layerBase
        const baseCount = this._layerBase._domContent.children.length;
        const newId = sortedNewId + baseCount;
        const oldId = sortedOldId + baseCount;

        const data = this._assets[0].get('data');
        const prevData = Object.assign({}, data);
        const viewingMovedLayer = this._parent._view._selectedLayer === Number(oldId);
        const layerValues = Object.values(data.layers);
        const layerMove = layerValues.splice(oldId, 1)[0];
        layerValues.splice(newId, 0, layerMove);
        Object.assign(data.layers, layerValues);

        /**
         * N.B. When updating layers transitions will change and then change back (not sure why...)
         * so need to suppress graph data events when setting data.
         * Entire data object needs to updated otherwise transitions will not change back and
         * graph generation will break on changing which layer to view (also not sure why...).
         */
        const redo = () => {
            this._assets[0].history.enabled = false;
            this._parent._view._suppressGraphDataEvents = true;
            this._assets[0].set('data', data);
            this._parent._view._suppressGraphDataEvents = false;
            if (viewingMovedLayer) {
                this._parent._view.link(this._assets, newId);
                this._updateLayerSelect(true);
            }
            this._assets[0].history.enabled = true;
        };
        const undo = () => {
            ...
            this._parent._view._suppressGraphDataEvents = false;
            if (viewingMovedLayer) {
                this._parent._view.link(this._assets, oldId);
                this._updateLayerSelect(true);
            }
            this._assets[0].history.enabled = true;
        };
    ...
    link(assets: Observer[]) {
        ...
        Object.keys(layers).forEach((layerKey, i) => {
            ...
            const stateNamePathRegex = /^data\.states\.\d*\.name$/;
            this._assetEvents.push(
            ...
            if (layerPanel.sortable) {
                this._layerSort.append(layerPanel);
            } else {
                this._layerBase.append(layerPanel);
            }
    ...
    unlink() {
        ...
        if (this._layerPanels) {
            this._layerPanels.forEach((layerPanel) => {
                if (layerPanel.sortable) {
                    this._layerSort.remove(layerPanel);
                } else {
                    this._layerBase.remove(layerPanel);
                }
            });
            this._layerPanels = [];
        }
