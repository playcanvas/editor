...
import { BUTTON_TRANSITION_MODE_TINT, BUTTON_TRANSITION_MODE_SPRITE_CHANGE } from 'playcanvas';
...
const ATTRIBUTES: Attribute[] = [
    ...
    },
    {
        ...
        args: {
            ...
            options: [
                {
                    v: BUTTON_TRANSITION_MODE_TINT,
                    t: 'Tint'
                },
                {
                    v: BUTTON_TRANSITION_MODE_SPRITE_CHANGE,
                    t: 'Sprite Change'
                }
            ]
        }
...
class ButtonComponentInspector extends ComponentInspector {
    ...
    _toggleFields() {
        ...
        ['hoverTint', 'pressedTint', 'inactiveTint', 'fadeDuration'].forEach((name) => {
            this._field(name).parent.hidden = !isTintMode;
        });

        ['hoverSpriteAsset', 'pressedSpriteAsset', 'inactiveSpriteAsset'].forEach((name) => {
            this._field(name).hidden = !isSpriteChangeMode;
        });

        ['hoverSpriteFrame', 'pressedSpriteFrame', 'inactiveSpriteFrame'].forEach((name) => {
    ...
    link(entities: EntityObserver[]) {
        ...
        const updateInputWarning = () => {
            this._inputWarning.hidden =
                entities.length !== 1 ||
                entities[0].get('components.element.useInput') ||
                !entities[0].get('components.button.active');
        };
