...
editor.once('load', () => {
    ...
    editor.method('picker:conflictManager:showSceneConflicts', (parent, conflicts, mergeObject) => {
        // create resolver
        ...
        if (index.missingInDst || index.missingInSrc) {
            const sectionScene = resolver.createSection(conflicts.itemName);
        ...
        // Scene properties
        const sectionProperties = resolver.createSection('PROPERTIES');
        ...
        if (index.entities) {
            ...
            let allowSectionCloaking = false;
            if (resolver.isDiff) {
                const numEntities = Object.keys(index.entities).length;
                allowSectionCloaking = numEntities > 50;
            }

            for (const key in index.entities) {
                // create title for entity section
                let entityName = resolver.srcEntityIndex[key] || resolver.dstEntityIndex[key];
                if (entityName) {
                ...
                const sectionEntity = resolver.createSection(entityName, true, allowSectionCloaking);
                ...
                if (entity.components) {
                    for (const component in componentSchema) {
                        ...
                        if (component === 'script') {
                            ...
                            const slots = entity.components.sound.slots;
                            if (slots) {
                            ...
                            const clips = entity.components.sprite.clips;
                            if (clips) {
                            ...
                            // handle mapping
                            const mapping = entity.components.model.mapping;
                            if (mapping) {
