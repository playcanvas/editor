...
editor.once('load', () => {
    ...
    editor.method(
        ...
        (
            ...
            templateInstanceRoot: Observer | null
        ): Observer[] => {
            ...
            while (entity) {
                if (entity.get('template_id') && entity.has(`template_ent_ids.${override.resource_id}`)) {
                    ...
                    if (
                        entity.get('resource_id') !== override.resource_id ||
                        !editor.call('template:utils', 'isIgnoreRootOverride', override.path)
                    ) {
                        result.push(entity);
                    }
                }
    ...
    editor.method(
        ...
        (
            templateInstanceRoot: Observer,
            deletedEntityData: Record<string, unknown>,
            entities: ObserverList
        ): Observer[] => {
            entities = entities || editor.call('entities:raw');
