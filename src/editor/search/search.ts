editor.once('load', () => {
    ...
    editor.method('search:charsContains', (a, b) => {
        if (a === b) {
            return a.length;
        }
    ...
    const searchItems = function (items, search, args) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const contains = editor.call('search:charsContains', search, item.name);
            ...
            for (let t = 0; t < item.tokens.length; t++) {
                ...
                const edits = editor.call('search:stringEditDistance', search, item.tokens[t]);
    ...
    // ]
    //
    editor.method('search:items', (items, search, args) => {
        ...
        const searchTokens = editor.call('search:stringTokenize', search);
