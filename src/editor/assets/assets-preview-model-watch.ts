...
editor.once('load', () => {
    ...
    const loadModel = function (watch, asset, reload) {
        ...
        if (file && file.url) {
            ...
            url = buildQueryUrl(url, { t: file.hash });
        }

        if (url && (reload || !asset._editorPreviewModel)) {
            app.assets._loader.load(url, asset.type, (err, resource, extra) => {
    ...
    const subscribe = function (watch) {
        ...
        watch.watching.file = watch.asset.on('file.hash:set', () => {
            setTimeout(onChange, 0);
        });

        watch.watching.fileUnset = watch.asset.on('file.hash:unset', () => {
            setTimeout(onChange, 0);
        });
