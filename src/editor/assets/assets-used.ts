editor.once('load', () => {
    ...
    const keys = {
        ...
        'material': {
            ...
            'data.specularityFactorMap': true,
            ...
            'data.glossMap': true,
            'data.clearCoatMap': true,
            'data.clearCoatGlossMap': true,
            'data.clearCoatNormalMap': true,
            'data.sheenMap': true,
            'data.sheenGlossMap': true,
            'data.refractionMap': true,
            'data.thicknessMap': true,
    ...
    const updateAsset = function (referer, type, oldId, newId) {
        ...
        if (newId) {
            ...
            if (!index[newId].ref[referer]) {
                ...
                index[newId].ref[referer][0] = editor.on(`assets:used:${referer}`, (state) => {
                    if (!index[newId]) {
                        return;
                    }
                ...
                if (item) {
                    index[newId].ref[referer][1] = item.once('destroy', () => {
    ...
    editor.on('assets:scripts:primary:set', (asset, script) => {
    ...
    editor.on('assets:scripts:primary:unset', (asset, script) => {
        const entities = editor.call('entities:list:byScript', script);
        const len = entities.length;
        if (!len) {
            return;
    ...
    sceneSettings.on('render.skybox:set', (value, valueOld) => {
    ...
    editor.method('assets:used:get', (id) => {
        if (isNaN(id)) {
            return true;
        }
