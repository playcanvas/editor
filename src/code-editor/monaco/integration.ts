editor.once('load', () => {
    ...
    const highlight = function (
        id: string,
        options: { line?: number; col?: number; error?: boolean; callback?: () => void }
    ) {
    ...
    const selectAndHighlight = function (
        id: string,
        options?: { line?: number; col?: number; error?: boolean; callback?: () => void }
    ) {
        if (events[id]) {
        ...
        if (!editor.call('views:get', id)) {
            events[id] = editor.once(`views:new:${id}`, () => {
                delete events[id];
    ...
    editor.method('integration:selectWhenReady', (id, options) => {
        if (filePanelReady) {
