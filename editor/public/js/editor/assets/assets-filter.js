editor.once('load', function() {
    ...
    var tooltipSearch = Tooltip.attach({
        ...
        hoverable: true,
        text: 'Search Assets',
        html: '<h1>Assets Search</h1><p>You can perform a global search for assets in your project using this Search box. Simply start typing into the box and the Editor will show matching results dynamically in the panel bellow.</p><p><strong>ID</strong> - Specific asset can be found by it\'s unique ID, by simply typing ID in search field it will recognize exact match and only show one asset with that ID.</p><p><strong>RegExp</strong> - It is possible to search using regular expressions. Add <code>*</code> at the beginning of search field and type regexp query after. To search for all assets use <code>*.</code> (any character) regexp query.</p><p><strong>Tags</strong> - To search by tags and their combinations type tags in square brackets <code>[ ]</code>. Simple query operators: AND, OR are allowed by expressing query as array of strings or other arrays with strings. Logic of query is same as for <a href="https://developer.playcanvas.com/en/api/pc.AssetRegistry.html#findByTag" target="_blank">findByTag</a> from <b>AssetsRegistry</b>.</p><p>Here are some examples:</p><p><code>[ level-1 ]</code> - returns all assets that are tagged by <code>level-1</code>.<br /><code>[ level-1, level-2 ]</code> - returns all assets that are tagged by <code>level-1 OR level-2</code>.<br /><code>[ [ level-1, monster ] ]</code> - returns all assets that are tagged by <code>level-1 AND monster</code>. Notice extra brackets.<br /><code>[ [ level-1, monster ], [ level-2, monster ] ]</code> - returns all assets that are tagged by <code>(level-1 AND monster) OR (level-2 AND monster)</code>.</p>'
    });
