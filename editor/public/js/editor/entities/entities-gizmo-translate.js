editor.once('load', function() {
    ...
    var onGizmoStart = function(axis, plane) {
        ...
        for(var i = 0; i < items.length; i++) {
            var pos = items[i].obj.get('position');
            items[i].start[0] = pos[0];
            items[i].start[1] = pos[1];
            items[i].start[2] = pos[2];
    ...
    var onGizmoOffset = function(x, y, z) {
        for(var i = 0; i < items.length; i++)
            items[i].obj.set('position', [ items[i].start[0] + x, items[i].start[1] + y, items[i].start[2] + z ]);
    ...
    var updateGizmo = function() {
        ...
        if (editor.call('selector:type') === 'entity' && editor.call('gizmo:type') === 'translate') {
            for(var i = 0; i < objects.length; i++) {
                ...
                if (items.length === 1 && coordSystem === 'local') {
                    var rot = items[0].obj.get('rotation');
                    editor.call('gizmo:translate:rotation', rot[0], rot[1], rot[2]);
                } else {
                    editor.call('gizmo:translate:rotation', 0, 0, 0);
                }
            }
