...
(function(mod) {
  ...
  cmds[map[selectLinesCombo + "Up"] = "selectLinesUpward"] = function(cm) {
    cm.operation(function() {
      // modified by Vaios to select up to the
      // maximum character on each line like Sublime does
      var ranges = cm.listSelections();
      var maxCh = -1;
      for (var i = 0; i < ranges.length; i++) {
        var range = ranges[i];
        if (range.head.line > cm.firstLine())
          if (range.head.ch > maxCh)
            maxCh = range.head.ch;
      }
      for (var i = 0; i < ranges.length; i++) {
        var range = ranges[i];
        if (range.head.line > cm.firstLine())
          cm.addSelection(Pos(range.head.line - 1, maxCh));
      }
    });
  };
