...
class ScriptAssetInspector extends Panel {
    ...
    _displayScriptAttributes() {
        ...
        Object.keys(scripts).forEach((scriptName) => {
            ...
            if (hasCollision)
                this._scriptAttributeContainer[`_${scriptName}Container`].append(new Label({ text: `script ${scriptName} is already defined in other asset`, class: [CLASS_SCRIPT, pcui.CLASS_ERROR] }));
    ...
    _onClickParse() {
        ...
        editor.call('scripts:parse', this._asset, (error, result) => {
            ...
            if (error) {
                ...
                this._errorContainer.append(new Label({ text: error.message, class: [CLASS_SCRIPT, pcui.CLASS_ERROR] }));
                return;
            }
            if (result.scriptsInvalid.length > 0) {
                this._errorContainer.append(new Label({ text: 'Validation Errors: ', class: [CLASS_SCRIPT, pcui.CLASS_ERROR] }));
                result.scriptsInvalid.forEach((invalidScript) => {
                    this._errorContainer.append(new Label({ text: invalidScript, class: [CLASS_SCRIPT, pcui.CLASS_ERROR] }));
                });
                this._errorContainer.hidden = false;
                return;
            }
            for (const scriptName in result.scripts) {
                ...
                if (attrInvalid.length > 0) {
                    const label = new Label({ text: attrInvalid, class: [pcui.CLASS_ERROR, CLASS_SCRIPT] });
